...
img_data_t *decodeTwoSteps(const char* filename) {
  ...
  img_data->height = height;
    ...
    if (debug) {
    for (int i = 0; i < 3 && i < img_data->data_size; ++i) {
...
txt_data_t *get_text_data(const char *filename, int img_size) {
  ...
  if (text == NULL) {
    free(text_data);
    printf("Error- Unable to allocate text data memory\n");
    exit ( -1 );
  }
  ...
  return text_data;
    // Caller is responsible for freeing the text_data (and ->data) memory
}
